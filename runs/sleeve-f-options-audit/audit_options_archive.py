...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--as-of", type=date.fromisoformat, default=date.today(), help="reference date for gap_to_today_days (default: today)")
    return p.parse_args()
...
def cadence(df: pd.DataFrame) -> dict:
    ...
    gaps = [(b - a).total_seconds() / 60 for a, b in pairwise(ts)]
...
def main() -> None:
    ...
    par_by_date = defaultdict(list)
    for p in csvs:
        csv_by_date[file_date(p)].append(p)
    for p in pars:
        par_by_date[file_date(p)].append(p)
    day_stats = []
    parse_bad = []
    csv_loaded = {}
    parquet_rows = {}
    ...
    overlap = sorted(set(csv_by_date) & set(par_by_date) & set(sampled))
    seam = []
    for d in overlap:
        c = pd.concat([read_csv_day(p) for p in csv_by_date[d]], ignore_index=True)
        c, _ = parse_symbols(c)
        c["close_num"] = pd.to_numeric(c["close"], errors="coerce")
        c = c.sort_values("timestamp").drop_duplicates(["symbol"], keep="last")
